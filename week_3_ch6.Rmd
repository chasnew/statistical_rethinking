...
# post-treatment bias creates a false association (collider bias)
set.seed(71)
...
# agent-based simulation of aging and marriage
d <- sim_happiness( seed=1977 , N_years=1000 )
precis(d)

d2 <- d[ d$age>17 , ] # only adults
d2$A <- ( d2$age - 18 ) / ( 65 - 18 ) # age scaling: 18 = 0 & 65 = 1

d2$mid <- d2$married + 1 # 1 = single, 2 = married

# model with collider bias
m6.9 <- quap(
  alist(
...
# appropriate model for the question
m6.10 <- quap(
  alist(
    happiness ~ dnorm( mu , sigma ),
    mu <- a + bA*A,
...
# a model if U (neighborhood effect) is observed
m6.12 <- quap(
  alist(
    C ~ dnorm( mu , sigma ),
